...
# Backward-compat alias: some modules expect `from inference.cell_count import detect`
# where `detect.run(...)` is used.
detect = sys.modules[__name__]
...
def run(weights='yolov5s.pt',  # model.pt path(s)
        ...
        project='tmp/markers',  # save results to project/name
        name='cell_count',  # save results to project/name
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            txt_path = str(save_dir / 'cells') + ('' if dataset.mode == 'image' else f'_{frame}')  # img.txt
...
def parse_opt():
    ...
    parser.add_argument('--hide-labels', default=True, action='store_true')
    parser.add_argument('--hide-conf', default=True, action='store_true')
